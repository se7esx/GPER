...
for key in datasets:
    ...
    if key=="mutag":
        ...
        args.borf_batch_add = 25
        args.borf_batch_remove = 3
    elif key=="enzymes":
        args.num_iterations = 1
        ...
        args.borf_batch_add = 1
        args.borf_batch_remove = 1
    elif key=="proteins":
        args.num_iterations = 3
        args.borf_batch_add = 0
        args.borf_batch_remove = 1
    elif key=="reddit":
        args.num_iterations = 3
        args.borf_batch_add = 4
        args.borf_batch_remove = 1
    elif key=="collab":
        args.num_iterations = 0
        args.borf_batch_add = 0
        args.borf_batch_remove = 0
    train_accuracies = []
    ...
    with tqdm.tqdm(total=len(dataset)) as pbar:
        if args.rewiring == "fosr":
            ...
            for i in range(len(dataset)):
                dataset[i].edge_index, dataset[i].edge_type = resistance_curvature.gerc3(dataset[i], 
                        loops=args.num_iterations, 
                        remove_edges=False, 
                        is_undirected=True,
                        batch_add=args.borf_batch_add,
    ...
    with open(f'results/new_graph_classification_{args.layer_type}_{args.rewiring}.csv', 'a') as f:
